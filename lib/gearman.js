...
var syslog        = syslog;
var syslogConsole = require('syslog-console');

if (syslogConsole) {
    syslog = syslogConsole.init('Gearman');
}
...
function Gearman(host, port, debug){
    ...
    if (Array.isArray(host)) {
        ...
        var settings;
        if (cluster.isWorker) {
            settings = host[cluster.worker.id % host.length];
            syslog.log({action: 'gearman.lib.host.modulo', host: settings, worker: cluster.worker.id});
        } else {
            settings = host[Math.floor(Math.random() * host.length)];
            syslog.log({action: 'gearman.lib.host.random', host: settings});
        }
    ...
    syslog.log({action: 'gearman.lib.host', host: this.host});
    this.init();
}
...
Gearman.prototype.sendCommandToServer = function(){
    ...
    if(this.debug){
        syslog.log("Sending: " + commandName + " with "+args.length+" params");
        syslog.log(" - ", body);
        args.forEach(function(arg, i){
            syslog.log("  - ARG:"+i+" ", arg.toString());
        });
    }
...
Gearman.prototype.receive = function(chunk){
    ...
    if(this.debug){
        syslog.log("Received: " + commandName + " with " + args.length + " params");
        syslog.log(" - ", data);
        args.forEach(function(arg, i){
            syslog.log("  - ARG:"+i+" ", arg.toString());
        });
    ...
    if(this.remainder && this.remainder.length>=12){
        setImmediate(this.receive.bind(this));
    }
};
...
Gearman.prototype.receive_ERROR = function(code, message){
    if(this.debug){
        syslog.log("Server error: ", code, message);
    }
};
...
Gearman.prototype.receive_WORK_STATUS = function(handle, numerator, denominator){
    if(this.currentJobs[handle] && !this.currentJobs[handle].aborted){
        this.currentJobs[handle].emit("status", numerator, denominator);
        this.currentJobs[handle].updateTimeout();
    }
};
...
Gearman.prototype.receive_JOB_ASSIGN_UNIQ = function(handle, name, unique, payload){
    if(typeof this.workers[name] == "function"){
        var worker = new this.Worker(this, handle, name, unique, payload);
        this.currentWorkers[handle] = worker;
        this.workers[name](payload, worker);
    }
};

Gearman.prototype.unregisterWorker = function(name){
...
Gearman.prototype.getStatus = function(sHandle){
    syslog.log("getStatus", sHandle);
    this.sendCommand("GET_STATUS", sHandle);
}
...
Gearman.prototype.Worker = function(gearman, handle, name, unique, payload){
    Stream.call(this);
...
Gearman.prototype.Worker.prototype.status = function(numberator, denominator){
    if(this.finished){
        return;
    }
    this.gearman.sendCommand("WORK_STATUS", this.handle, numberator, denominator);
};
...
Gearman.prototype.Worker.prototype.endAndGrabJob = function(data){
    this.end(data);
    this.gearman.sendCommand("GRAB_JOB_UNIQ");
};

...
function Gearman(hosts, debug){
    Stream.call(this);

    this.hosts = hosts;
    this.debug = debug !== undefined ? debug : true;

    syslog.debug({action: 'gearman.hosts', hosts: this.hosts});
    this.init();
}
...
Gearman.prototype.getStatus = function(handle){
    if (this.gearman.debug) syslog.debug({action: 'gearman.status', handle: handle});
...
Gearman.prototype._clearJobs = function(){
    for(var i in this.activeJobs){
        if(this.activeJobs[i] !== undefined){
            ...
            this.activeJobs[i].emit("error", new Error("Job failed"));
        }
...
Gearman.prototype._clearWorkers = function(){
    for(var i in this.activeWorkers){
        if(this.activeWorkers[i] !== undefined){
            this.activeWorkers[i].finished = true;
        }

        delete this.activeWorkers[i];
    }
};

Gearman.prototype.hasActiveWorker = function() {
    return Object.keys(this.activeWorkers).length > 0;
};
...
Gearman.prototype._workerAssign = function(connection, handle, name, unique, payload) {
    if (typeof this.workers[name] == 'function') {
        if (this.debug) syslog.debug({action: 'gearman._workerAssign', handle: handle, name: name, payload: payload});

        var worker = new this.Worker(this, connection, handle, name, unique, payload);
        this.activeWorkers[handle] = worker;
        this.workers[name](payload, worker);
    }
};

Gearman.prototype._workerRemove = function(handle) {
    if (this.activeWorkers[handle]) {
        delete this.activeWorkers[handle];
    }
...
Gearman.prototype.Connection.prototype.sendCommand = function(){
    if (this.gearman.debug) syslog.debug({action: 'gearman.connection.sendCommand', host: this.host});
...
Gearman.prototype.Connection.prototype._connected = function() {
    ...
    if (this.debug) syslog.debug({action: 'gearman.connection.connected', host: this.host});
...
Gearman.prototype.Connection.prototype._error = function(err){
    if (this.gearman.debug) syslog.debug({action: 'gearman.connection._error', host: this.host, error: e});
...
Gearman.prototype.Connection.prototype._processCommandQueue = function(chunk){
    if (this.gearman.debug) syslog.debug({action: 'gearman.connection._processCommandQueue', host: this.host, chunk: chunk});
...
Gearman.prototype.Connection.prototype._sendCommandToServer = function(){
    ...
    if(this.gearman.debug){
        syslog.debug({action: 'gearman.connection._sendCommandToServer', command: commandName, args: args.toString('utf8'), body: body.toString('utf8')});
    }
...
Gearman.prototype.Connection.prototype._receive = function(chunk){
    if (this.gearman.debug) syslog.debug({action: 'gearman.connection._receive', host: this.host, chunk: chunk});
    ...
    if(this.debug){
        syslog.debug({action: 'gearman.connection._receive', command: commandName, args: args, data: data});
    }
...
Gearman.prototype.Connection.prototype.receive_NO_JOB = function(){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_no_job', host: this.host});
...
Gearman.prototype.Connection.prototype.receive_NOOP = function(){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_noop', host: this.host});

    if (!this.gearman.hasActiveWorker()) {
        this.sendCommand("GRAB_JOB_UNIQ");
    }
};

Gearman.prototype.Connection.prototype.receive_ECHO_REQ = function(payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_echo_req', host: this.host, payload: payload});
    this.sendCommand("ECHO_RES", payload);
};

Gearman.prototype.Connection.prototype.receive_ERROR = function(code, message){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_error', host: this.host, code: code, message: message});
};
...
Gearman.prototype.Connection.prototype.receive_WORK_FAIL = function(handle){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_work_fail', host: this.host, handle: handle});

    this.gearman._jobFail(handle);
};

Gearman.prototype.Connection.prototype.receive_WORK_DATA = function(handle, payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_work_data', host: this.host, handle: handle, payload: payload});
...
Gearman.prototype.Connection.prototype.receive_WORK_STATUS = function(handle, numerator, denominator){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_work_status', host: this.host, handle: handle, numerator: numerator, denominator: denominator});
...
Gearman.prototype.Connection.prototype.receive_WORK_COMPLETE = function(handle, payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_work_complete', host: this.host, handle: handle, payload: payload});
...
Gearman.prototype.Connection.prototype.receive_JOB_ASSIGN_UNIQ = function(handle, name, unique, payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_job_assing_uniq', host: this.host, handle: handle, name: name, unique: unique});
...
Gearman.prototype.Connection.prototype.receive_JOB_ASSIGN = function(handle, name, payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_job_assign', host: this.host, handle: handle, name: name});
...
Gearman.prototype.Worker.prototype.endAndGrabJob = function(data){
    this.end(data);
    this.connection.sendCommand("GRAB_JOB_UNIQ");
};
...
Gearman.prototype.Worker.prototype._finish = function(){
    if(this.finished){
        return;
    }
...
Gearman.prototype.Job = function(gearman, name, payload, uniq, options){
    ...
    this.gearman.broadcastCommand(jobType, name, uniq ? uniq : false, payload, !options.background ? this.receiveHandle.bind(this) : false);
};
...
Gearman.prototype.Job.prototype.fail = function() {
    if(!this.aborted){
        this.abort();
        this.emit("error", new Error("Job failed"));
    }
};

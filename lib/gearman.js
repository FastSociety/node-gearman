...
Gearman.prototype.Connection.prototype._sendCommandToServer = function(){
    ...
    if(this.gearman.debug){
        syslog.debug({action: 'gearman.connection._sendCommandToServer', command: commandName, args: args.toString('utf8')});
    }
...
Gearman.prototype.Connection.prototype._receive = function(chunk){
    if (this.gearman.debug) syslog.debug({action: 'gearman.connection._receive', host: this.host});
    ...
    if(this.debug){
        syslog.debug({action: 'gearman.connection._receive', command: commandName, args: args});
    }
...
Gearman.prototype.Connection.prototype.receive_WORK_COMPLETE = function(handle, payload){
    if(this.debug) syslog.debug({action: 'gearman.connection.receive_work_complete', host: this.host, handle: handle});

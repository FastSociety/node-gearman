...
Gearman.prototype.Connection.prototype._sendCommandToServer = function(){
    ...
    if(this.gearman.debug) syslog.debug({action: 'gearman.connection._sendCommandToServer', command: commandName, args: args.toString('utf8')});
...
Gearman.prototype.Connection.prototype.receive_JOB_ASSIGN_UNIQ = function(handle, name, unique, payload){
    if(this.gearman.debug) syslog.debug({action: 'gearman.connection.receive_job_assing_uniq', host: this.host, handle: handle, name: name, unique: unique});

...
Gearman.prototype._jobFail = function(handle, error) {
    if (this.debug) syslog.debug({action: 'gearman._jobFail', handle: handle, error: error});
...
Gearman.prototype.Job.prototype.fail = function(error) {
...
Gearman.prototype.Job.prototype.abort = function(sError){
    ...
    if (sError) {
        ...
        this.emit("error", new Error('No error message given'));
    }
};
